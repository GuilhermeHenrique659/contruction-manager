...
describe('CreateVendor', () => {
    it('given new vendor when execute then creates vendor', async () => {
        ...
        const result = await useCase.execute({ name: 'Vendor A', paymentDay: new DayOfMonth(10).value, projectId: 'proj-1', userId: 'u1' });
    ...
    it('given duplicate name for same project when execute then throws', async () => {
        const repo = new FakeVendorRepository();
        const useCase = new CreateVendor(repo);
        await useCase.execute({ name: 'Vendor A', paymentDay: new DayOfMonth(10).value, projectId: 'proj-1', userId: 'u1' });

        await assert.rejects(
            () => useCase.execute({ name: 'Vendor A', paymentDay: new DayOfMonth(15).value, projectId: 'proj-1', userId: 'u1' }),
            /already exists/
        );
    });
});
